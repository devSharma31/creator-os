 ...
 * 2. Run: npm create vite@latest creator-os -- --template react
 * 3. Run: npm install recharts
 * 4. Save this file as src/App.jsx
 * 5. npm run dev  →  localhost:5173
 * 6. npm install -g vercel && vercel  →  live URL
 * ─────────────────────────────────────────────────────────────
 */
...
/* ─── CONFIG — REPLACE THIS ─────────────────────────────────── */
const GOOGLE_CLIENT_ID = "183861502552-khpjlafupti51u11va5jufc3d5qj27kb.apps.googleusercontent.com";
const YT_SCOPES = [
...
:root{
  --bg:#07080f;--s1:#0e1018;--s2:#141720;--s3:#1c1f2e;
  --ig:linear-gradient(135deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888);
  --igs:#e1306c;--yt:#ff2b2b;--ytd:#cc0000;
  --pu:#a855f7;--bl:#3b82f6;--gr:#22c55e;--yw:#eab308;
...
.auth-gem{width:56px;height:56px;border-radius:17px;margin:0 auto 22px;font-size:25px;
  ...
  background:var(--ig);box-shadow:0 10px 34px rgba(225,48,108,.5)}
.auth-title{font-family:var(--fh);font-size:29px;font-weight:800;letter-spacing:-.6px}
.auth-sub{color:var(--t2);font-size:14px;margin-top:8px;line-height:1.65}
.auth-div{display:flex;align-items:center;gap:12px;margin:26px 0;color:var(--t3);font-size:11px;letter-spacing:.5px}
...
.yt-btn:hover{border-color:var(--yt);background:rgba(255,43,43,.15);transform:translateY(-1px)}
.s-btn:disabled{opacity:.35;cursor:not-allowed;transform:none}
.divider-text{font-size:11.5px;color:var(--t3);margin-top:16px;line-height:1.7}

/* CONNECT */
.conn-bg{height:100vh;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:28px;
  background:radial-gradient(ellipse 80% 55% at 50% -5%,rgba(188,24,136,.16),transparent),var(--bg)}
.conn-title{font-family:var(--fh);font-size:24px;font-weight:800;text-align:center}
.conn-sub{color:var(--t2);font-size:14px;text-align:center;max-width:400px;line-height:1.65}
.pc-wrap{display:flex;gap:18px}
.pc{background:var(--card);border:2px solid var(--bd);border-radius:22px;
  padding:32px 40px;text-align:center;cursor:pointer;transition:all .25s;min-width:190px;position:relative;overflow:hidden}
.pc::before{content:'';position:absolute;inset:0;opacity:0;transition:opacity .25s;pointer-events:none}
.pc.ig-pc::before{background:var(--ig)}
.pc.yt-pc::before{background:linear-gradient(135deg,#ff2b2b,#880000)}
.pc:hover:not(.done)::before{opacity:.09}
.pc:hover:not(.done){transform:translateY(-3px);border-color:var(--bd2)}
.pc.done{border-color:var(--gr);cursor:default}
.pc-icon{font-size:42px;margin-bottom:12px;position:relative}
.pc-name{font-family:var(--fh);font-size:15px;font-weight:700;margin-bottom:8px;position:relative}
.pc-status{font-size:12px;color:var(--t2);position:relative}
.pc.done .pc-status{color:var(--gr);font-weight:600}
.pc-hint{font-size:10.5px;color:var(--t3);margin-top:6px;position:relative}
.conn-cta{padding:14px 42px;background:var(--igs);border:none;border-radius:13px;color:#fff;
  font-family:var(--fh);font-size:13px;font-weight:700;letter-spacing:1px;
  cursor:pointer;transition:all .2s;box-shadow:0 10px 32px rgba(225,48,108,.38)}
.conn-cta:hover{filter:brightness(1.1);transform:translateY(-2px)}
.conn-cta:disabled{opacity:.28;cursor:not-allowed;transform:none;box-shadow:none}
.skip{font-size:12px;color:var(--t3);cursor:pointer;text-decoration:underline;text-underline-offset:3px}
...
.logo-gem{width:36px;height:36px;border-radius:11px;flex-shrink:0;display:flex;align-items:center;
  justify-content:center;font-size:17px;background:var(--ig);box-shadow:0 4px 18px rgba(225,48,108,.45)}
.logo-text{font-family:var(--fh);font-size:19px;font-weight:800;letter-spacing:-.3px}
.logo-ver{font-size:9px;color:var(--t3);letter-spacing:2px;text-transform:uppercase;margin-top:1px}
.user-pill{margin:13px 0 0;display:flex;align-items:center;gap:9px;padding:9px 11px;
  background:var(--s2);border:1px solid var(--bd);border-radius:11px}
.user-av{width:28px;height:28px;border-radius:50%;background:var(--ig);display:flex;align-items:center;
  justify-content:center;font-size:12px;font-weight:700;color:#fff;flex-shrink:0;overflow:hidden}
.user-av img{width:100%;height:100%;object-fit:cover}
.user-name{font-size:12.5px;font-weight:600;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.user-badge{font-size:9px;padding:2px 7px;border-radius:7px;font-weight:700;letter-spacing:.5px;
  background:linear-gradient(135deg,rgba(240,148,51,.2),rgba(188,24,136,.2));color:#e1306c}
.nav-area{flex:1;overflow-y:auto;padding:10px}
.nav-grp{font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--t3);padding:11px 10px 5px;font-family:var(--fh)}
.nav-btn{display:flex;align-items:center;gap:10px;padding:9.5px 11px;border-radius:11px;
  background:none;border:none;color:var(--t2);font-family:var(--fb);font-size:13px;font-weight:500;
  cursor:pointer;width:100%;text-align:left;transition:all .15s}
.nav-btn:hover{background:rgba(255,255,255,.05);color:var(--tx)}
.nav-btn.on{background:rgba(225,48,108,.16);color:var(--igs)}
.nav-btn .ni{font-size:15px;width:20px;text-align:center;flex-shrink:0}
.plat-row{padding:14px 20px;border-top:1px solid var(--bd);display:flex;gap:7px;flex-wrap:wrap}
.pp{display:flex;align-items:center;gap:5px;padding:4px 10px;border-radius:9px;font-size:10px;font-weight:600}
...
.ct{font-family:var(--fh);font-size:10px;font-weight:700;text-transform:uppercase;
  letter-spacing:2px;color:var(--t2);margin-bottom:17px;display:flex;align-items:center;gap:7px}
...
.btn-yt{background:var(--yt);color:#fff;box-shadow:0 6px 22px rgba(255,43,43,.3)}
.btn-yt:hover{filter:brightness(1.1);transform:translateY(-1px)}
.btn-gh{background:var(--s2);color:var(--t2);border:1px solid var(--bd)}
.btn-gh:hover{border-color:var(--igs);color:var(--igs)}
.btn-sm{padding:6px 15px;font-size:10px}
.btn-del{background:rgba(255,43,43,.1);color:var(--yt);border:1px solid rgba(255,43,43,.2)}
...
.g3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:17px}
.g4{display:grid;grid-template-columns:repeat(4,1fr);gap:15px}
.stat{background:var(--card);border:1px solid var(--bd);border-radius:var(--r);padding:19px 21px;position:relative;overflow:hidden}
.stat::after{content:'';position:absolute;top:0;left:0;right:0;height:2.5px}
.sig::after{background:var(--ig)}
.syt::after{background:linear-gradient(90deg,#ff2b2b,#880000)}
.spu::after{background:linear-gradient(90deg,var(--pu),#7c3aed)}
.sgr::after{background:linear-gradient(90deg,var(--gr),#16a34a)}
.syw::after{background:linear-gradient(90deg,var(--yw),#ca8a04)}
.s-lbl{font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:var(--t2);font-family:var(--fh);font-weight:600}
.s-val{font-family:var(--fh);font-size:29px;font-weight:800;margin:9px 0 3px;letter-spacing:-.5px}
.s-sub{font-size:11.5px;color:var(--t2)}
.s-good{color:var(--gr)}
...
/* TAGS & PILLS */
.tag{display:inline-block;padding:3px 9px;border-radius:20px;font-size:10px;font-weight:600;margin:2px}
.tig{background:rgba(225,48,108,.15);color:var(--igs)}
.tyt{background:rgba(255,43,43,.12);color:var(--yt)}
.tgr{background:rgba(34,197,94,.12);color:var(--gr)}
.tyw{background:rgba(234,179,8,.12);color:var(--yw)}
.tpu{background:rgba(168,85,247,.12);color:var(--pu)}
.tbl-tag{background:rgba(59,130,246,.12);color:var(--bl)}
.pill{display:inline-block;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:600}
.pok{background:rgba(34,197,94,.12);color:var(--gr)}
.pwn{background:rgba(234,179,8,.12);color:var(--yw)}
.pdn{background:rgba(255,43,43,.12);color:var(--yt)}
.pin{background:rgba(59,130,246,.12);color:var(--bl)}

/* SCORE BAR */
.sbar{height:5px;background:var(--s3);border-radius:3px;overflow:hidden;margin-top:7px}
.sbar-f{height:100%;border-radius:3px;background:var(--ig);transition:width 1.1s cubic-bezier(.16,1,.3,1)}

/* CALENDAR */
.cal-dn{font-size:9px;color:var(--t3);text-transform:uppercase;letter-spacing:1px;text-align:center;padding:5px 0;font-family:var(--fh)}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:5px;margin-bottom:5px}
.cal-cell{background:var(--s2);border:1px solid var(--bd);border-radius:9px;padding:8px;min-height:76px}
.cal-num{font-size:9px;color:var(--t3);font-family:var(--fh);margin-bottom:5px}
.chip{border-radius:5px;padding:2px 5px;font-size:9px;line-height:1.55;margin-bottom:2px}
.chip.ig{background:rgba(225,48,108,.18);color:var(--igs)}
.chip.yt{background:rgba(255,43,43,.15);color:var(--yt)}

/* NICHE CHIPS */
.nc{display:inline-flex;align-items:center;gap:6px;padding:7px 15px;border-radius:21px;
  border:1px solid var(--bd);font-size:12.5px;cursor:pointer;transition:all .15s;
  background:var(--s2);color:var(--t2);margin:3px;font-weight:500}
.nc:hover{border-color:var(--igs);color:var(--igs)}
.nc.on{border-color:var(--igs);color:var(--igs);background:rgba(225,48,108,.1)}
...
.video-thumb-img{width:100%;aspect-ratio:16/9;background:var(--s3);
  display:flex;align-items:center;justify-content:center;font-size:28px}
.video-info{padding:10px}
.video-title{font-size:12px;font-weight:600;line-height:1.5;margin-bottom:5px;
  display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.video-meta{font-size:10.5px;color:var(--t2)}
.loading-spinner{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px;gap:14px;color:var(--t2);font-size:13.5px}

/* AI TOOL ENHANCEMENTS */
.tool-tip{font-size:11.5px;color:var(--t3);margin-bottom:14px;line-height:1.6;padding:10px 13px;background:var(--s2);border-radius:9px;border-left:3px solid var(--igs)}
.copy-success{color:var(--gr);font-size:11px;font-style:normal}
.char-count{font-size:10.5px;color:var(--t3);text-align:right;margin-top:-12px;margin-bottom:10px}
...
/* ─── CONSTANTS ─────────────────────────────────────────────── */
const NICHES=[{e:"💪",l:"Fitness & Health"},{e:"💰",l:"Finance & Investing"},{e:"🎮",l:"Gaming"},{e:"✈️",l:"Travel & Lifestyle"},{e:"🍕",l:"Food & Cooking"},{e:"👗",l:"Fashion & Beauty"},{e:"💻",l:"Tech & AI"},{e:"📸",l:"Photography"},{e:"🎵",l:"Music & Entertainment"},{e:"📚",l:"Education & Coaching"},{e:"🏠",l:"Home & Interior"},{e:"💼",l:"Business & Marketing"}];

const NAV=[
  {g:"Overview",items:[{id:"dash",i:"⚡",l:"Dashboard"},{id:"analytics",i:"📊",l:"YT Analytics"}]},
  {g:"AI Tools",items:[
    {id:"growth",i:"📈",l:"Growth Strategist"},
    {id:"script",i:"🎬",l:"Script Writer"},
    {id:"caption",i:"✍️",l:"Caption & Hashtags"},
    {id:"hook",i:"🪝",l:"Hook Generator"},
    {id:"repurpose",i:"🔁",l:"Repurpose Engine"},
    {id:"viral",i:"🔥",l:"Viral Score Analyzer"},
    {id:"calendar",i:"📅",l:"Content Calendar"},
  ]},
  {g:"Business",items:[
    {id:"collab",i:"🤝",l:"Brand Collabs"},
    {id:"invoice",i:"🧾",l:"Invoice Generator"},
    {id:"earn",i:"💸",l:"Earnings Tracker"},
  ]},
];
...
function useAI() {
  ...
  const run = useCallback(async (prompt, sys) => {
    setLoading(true); setOut("");
    await callClaude(prompt, sys, txt => setOut(txt));
    setLoading(false);
  }, []);
  return { out, loading, run, setOut };
}
    ...
    return new Promise((resolve, reject) => {
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (response) => {
          ...
          await fetchYouTubeData(response.access_token, resolve, reject);
        },
      });
...
/* ─── AUTH SCREEN ───────────────────────────────────────────── */
function AuthScreen({ onYouTube, onSkip }) {
  const [loading, setLoading] = useState(false);
  return (
...
/* ─── ANALYTICS PAGE ────────────────────────────────────────── */
function Analytics({ ytData, ytUser, ytLoading, ytError, signIn, signOut }) {
            ...
            {growthData.length > 0 ? (
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={growthData}>
                  ...
                  <YAxis tick={{fill:"var(--t3)",fontSize:10}} axisLine={false} tickLine={false} width={44} tickFormatter={v=>fmt(v)}/>
                  <Tooltip contentStyle={{background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:9,fontSize:12}} formatter={v=>[fmt(v),"Views"]} itemStyle={{color:"var(--yt)"}}/>
                  <Bar dataKey="views" fill="url(#ytgrad)" radius={[5,5,0,0]}/>
                  <defs><linearGradient id="ytgrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ff2b2b"/><stop offset="100%" stopColor="#880000"/></linearGradient></defs>
                </BarChart>
              </ResponsiveContainer>
...
/* ─── DASHBOARD ─────────────────────────────────────────────── */
function Dashboard({ user, profile, setProfile, ytUser, nav }) {
  const [step, setStep] = useState(profile.niche ? 1 : 0);
  const [tn, setTn] = useState(profile.niche||"");
  const [h, setH] = useState(profile.handle || ytUser?.handle || "");

  if (step === 0) return (
    <div className="fade">
      <div className="ph"><div className="ph-title"><span>⚡</span> Welcome{ytUser?`, ${ytUser.name.split(" ")[0]}`:""}!</div><div className="ph-sub">Personalise your AI suite — takes 30 seconds</div></div>
      <div className="card">
        <div className="ct"><span className="d"></span>Your Creator Handle</div>
  ...
  return (
    <div className="fade">
      <div className="ph"><div className="ph-title"><span>⚡</span> Dashboard</div><div className="ph-sub">{profile.handle} · {profile.niche} Creator</div></div>
      <div className="g3" style={{marginBottom:17}}>
        <div className="stat syt"><div className="s-lbl">YT Subscribers</div><div className="s-val">{ytUser?fmt(ytUser.subscribers):"—"}</div><div className="s-sub">{ytUser?"Real-time data 🔴":"Connect YouTube"}</div></div>
        <div className="stat syt"><div className="s-lbl">Total Views</div><div className="s-val">{ytUser?fmt(ytUser.totalViews):"—"}</div><div className="s-sub">{ytUser?"All time":"Connect YouTube"}</div></div>
        <div className="stat spu"><div className="s-lbl">Videos Published</div><div className="s-val">{ytUser?ytUser.videoCount:"—"}</div><div className="s-sub">{ytUser?"On your channel":"Connect YouTube"}</div></div>
      </div>
      <div className="g2">
        <div className="card">
          <div className="ct"><span className="d"></span>Quick Launch</div>
          {[{i:"📅",l:"Generate 30-Day Calendar",p:"calendar"},{i:"🎬",l:"Write a Video Script",p:"script"},{i:"🔥",l:"Check Viral Score",p:"viral"},{i:"✍️",l:"Generate Captions",p:"caption"},{i:"📊",l:"View YT Analytics",p:"analytics"}].map(a=>(
            <div key={a.p} onClick={()=>nav(a.p)} style={{display:"flex",alignItems:"center",gap:11,padding:"10px 0",borderBottom:"1px solid var(--bd)",cursor:"pointer",color:"var(--t2)",fontSize:13.5,transition:"color .15s"}} onMouseOver={e=>e.currentTarget.style.color="var(--tx)"} onMouseOut={e=>e.currentTarget.style.color="var(--t2)"}>
              <span style={{fontSize:18}}>{a.i}</span>{a.l}<span style={{marginLeft:"auto",color:"var(--t3)"}}>→</span>
            </div>
          ))}
        </div>
        <div className="card">
...
function AIPage({ title, icon, sub, tip, fields, buildPrompt, sys, outLabel, extraActions }) {
  ...
  return (
    <div className="fade">
      <div className="ph"><div className="ph-title"><span>{icon}</span> {title}</div><div className="ph-sub">{sub}</div></div>
      <div className="g2">
        <div className="card">
          {tip && <div className="tool-tip">💡 {tip}</div>}
          <div className="ct"><span className="d"></span>Your Details</div>
          {fields.map(f=>(
            <div key={f.k}>
              <label className="lbl">{f.label}{!f.req?" (optional)":""}</label>
              {f.type==="select"
          ...
          <button className="btn btn-ig" disabled={loading||!canRun} onClick={()=>run(buildPrompt(vals),sys)}>
            {loading ? <><span className="spin">◌</span> Generating...</> : <>{icon} Generate</>}
          </button>
        </div>
...
/* ─── TOOL DEFINITIONS ──────────────────────────────────────── */
const GrowthPage = ({profile, ytUser}) => <AIPage
  title="Growth Strategist" icon="📈"
  sub="AI-powered 90-day roadmap engineered for your exact niche and platform"
  tip="The more specific your challenge, the better the strategy. Be honest about where you're stuck."
  fields={[
    {k:"followers",label:"Current Subscribers / Followers",ph:`e.g. ${ytUser?fmt(ytUser.subscribers):"5,000"}`,req:true},
...
const ScriptPage = ({profile}) => <AIPage
  title="Script Writer" icon="🎬"
  sub="Full retention-engineered scripts with timestamps, chapters, and B-roll notes"
  tip="The more specific your angle, the better the script. A good angle = 80% of the battle."
  fields={[
    {k:"topic",label:"Video Topic",ph:"e.g. I tried the viral 75 Hard challenge for 30 days",req:true},
    {k:"format",label:"Format",type:"select",opts:["YouTube Long-form (10-15 min)","YouTube Long-form (5-8 min)","YouTube Short (60s)","Instagram Reel (30-60s)","Instagram Reel (60-90s)"]},
        ...
        <div className="fade">
          <div className="g2" style={{marginBottom:17}}>
            <div className="card" style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6}}>
              ...
              <div style={{display:"flex",gap:7,marginTop:10,flexWrap:"wrap",justifyContent:"center"}}>
                {res.potential && <span className={`pill ${pot}`}>Potential: {res.potential}</span>}
                {res.best_platform && <span className="tag tyt">{res.best_platform}</span>}
              </div>
              {res.best_time && <div style={{marginTop:4}}><span className="tag tpu">📅 {res.best_time}</span></div>}
            </div>
            <div className="card">
          ...
          {res.title_suggestions?.length > 0 && (
            <div className="card">
              ...
              {res.similar_viral && <div style={{marginTop:14,padding:"10px 14px",background:"rgba(168,85,247,.08)",borderRadius:9,fontSize:12.5,color:"var(--pu)"}}>📺 Similar viral video: <strong>{res.similar_viral}</strong></div>}
            </div>
          )}
        </div>
      )}
...
Create a balanced mix: ~40% YouTube, ~60% Instagram. Vary content types and goals strategically.`,
  ...
  const weeks = [0,1,2,3].map(w=>Array.from({length:7},(_,i)=>posts[w*7+i]||null));
  const goalColor = g => g==="subscribers"?"var(--gr)":g==="views"?"var(--yt)":g==="engagement"?"var(--pu)":"var(--yw)";
...
/* ─── BRAND COLLABS ──────────────────────────────────────────── */
function CollabPage() {
  const [rows,setRows]=useState([{id:1,brand:"TechBrand India",type:"Sponsored Reel",val:"₹25,000",status:"Negotiating",dl:"15 Apr 2026",notes:"Awaiting contract"}]);
  const [form,setForm]=useState({brand:"",type:"Sponsored Reel",val:"",status:"Outreach",dl:"",notes:""});
  const [open,setOpen]=useState(false);
  const SF={Outreach:"pin",Negotiating:"pwn",Confirmed:"pok",Completed:"pok",Declined:"pdn"};
  const add=()=>{if(!form.brand)return;setRows(r=>[...r,{...form,id:Date.now()}]);setForm({brand:"",type:"Sponsored Reel",val:"",status:"Outreach",dl:"",notes:""});setOpen(false);};
  const total=rows.filter(r=>r.status==="Confirmed"||r.status==="Completed").reduce((s,r)=>s+Number(r.val.replace(/[₹,]/g,"")||0),0);
  return (
    <div className="fade">
      <div className="ph"><div className="ph-title"><span>🤝</span> Brand Collab Manager</div><div className="ph-sub">Track every brand deal from first DM to final payment</div></div>
      <div className="g3" style={{marginBottom:17}}>
        <div className="stat sig"><div className="s-lbl">Active Deals</div><div className="s-val">{rows.filter(r=>r.status==="Negotiating"||r.status==="Confirmed").length}</div><div className="s-sub">In progress</div></div>
        <div className="stat sgr"><div className="s-lbl">Confirmed Value</div><div className="s-val">₹{total.toLocaleString("en-IN")}</div><div className="s-sub">This month</div></div>
        <div className="stat spu"><div className="s-lbl">Total Deals</div><div className="s-val">{rows.length}</div><div className="s-sub">All time</div></div>
      </div>
      <div className="card">
        <div className="rb" style={{marginBottom:15}}><div className="ct" style={{marginBottom:0}}><span className="d"></span>Deal Pipeline</div><button className="btn btn-gh btn-sm" onClick={()=>setOpen(o=>!o)}>+ Add Deal</button></div>
        {open && <div className="fade" style={{background:"var(--s2)",padding:18,borderRadius:12,marginBottom:18}}>
          <div className="g3">
            <div><label className="lbl">Brand Name</label><input placeholder="Brand / Company" value={form.brand} onChange={e=>setForm(f=>({...f,brand:e.target.value}))}/></div>
            <div><label className="lbl">Deal Type</label><select value={form.type} onChange={e=>setForm(f=>({...f,type:e.target.value}))}>{["Sponsored Reel","Sponsored Video","UGC Package","Brand Ambassador","Product Review","Affiliate","Speaking/Event"].map(t=><option key={t}>{t}</option>)}</select></div>
...
/* ─── INVOICE GENERATOR ──────────────────────────────────────── */
function InvoicePage({ profile }) {
  const {out,loading,run}=useAI();
  const [f,setF]=useState({to:"",email:"",services:"",amount:"",gst:"",due:"",notes:"",invoiceNo:""});
...
Format this as a clean, professional text invoice. Include a brief professional closing note.`)}>
        ...
        <div className="card">
          <div className="ct"><span className="d"></span>Your Invoice</div>
...
function EarnPage() {
  const [rows,setRows]=useState([
    {id:1,src:"TechBrand India — Sponsored Reel",type:"Sponsorship",amt:25000,mo:"Mar 2026",st:"Paid"},
    {id:2,src:"YouTube AdSense",type:"AdSense",amt:4200,mo:"Mar 2026",st:"Paid"},
    {id:3,src:"Amazon Affiliate Program",type:"Affiliate",amt:1850,mo:"Mar 2026",st:"Pending"},
    {id:4,src:"FashionBrand — UGC Package",type:"UGC",amt:12000,mo:"Apr 2026",st:"Confirmed"},
  ]);
  const [form,setForm]=useState({src:"",type:"Sponsorship",amt:"",mo:"",st:"Paid"});
  const [open,setOpen]=useState(false);
  const fmt2=n=>`₹${Number(n).toLocaleString("en-IN")}`;
  const total=rows.reduce((s,r)=>s+Number(r.amt),0);
  const paid=rows.filter(r=>r.st==="Paid").reduce((s,r)=>s+Number(r.amt),0);
  const pend=rows.filter(r=>r.st!=="Paid"&&r.st!=="Declined").reduce((s,r)=>s+Number(r.amt),0);
  const mdata=Object.values(rows.reduce((a,r)=>{a[r.mo]=a[r.mo]||{m:r.mo,t:0};a[r.mo].t+=Number(r.amt);return a;},{}));
  const typeData=Object.values(rows.reduce((a,r)=>{a[r.type]=a[r.type]||{name:r.type,value:0};a[r.type].value+=Number(r.amt);return a;},{}));
  const PIE_COLORS=["#e1306c","#ff2b2b","#a855f7","#22c55e","#3b82f6"];
  ...
  return (
    <div className="fade">
      ...
      <div className="g4" style={{marginBottom:17}}>
        ...
        <div className="stat spu"><div className="s-lbl">Pending</div><div className="s-val" style={{color:"var(--yw)"}}>{fmt2(pend)}</div><div className="s-sub">Awaiting payment</div></div>
        <div className="stat syw"><div className="s-lbl">Avg Per Deal</div><div className="s-val">{rows.length?fmt2(Math.round(total/rows.length)):"₹0"}</div><div className="s-sub">Per entry</div></div>
      </div>
      <div className="g2">
        <div className="card">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:17}}>
          <div className="card">
            <div className="ct"><span className="d"></span>Monthly Earnings</div>
            <ResponsiveContainer width="100%" height={170}>
              <BarChart data={mdata}>
                <XAxis dataKey="m" tick={{fill:"var(--t3)",fontSize:10}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fill:"var(--t3)",fontSize:10}} axisLine={false} tickLine={false} width={55} tickFormatter={v=>`₹${(v/1000).toFixed(0)}K`}/>
                <Tooltip contentStyle={{background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:9,fontSize:12}} formatter={v=>[`₹${v.toLocaleString("en-IN")}`,""]} labelStyle={{color:"var(--t2)"}}/>
                <Bar dataKey="t" fill="url(#earngrad)" radius={[5,5,0,0]}/>
                <defs><linearGradient id="earngrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#e1306c"/><stop offset="100%" stopColor="#bc1888"/></linearGradient></defs>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="card">
            <div className="ct"><span className="d"></span>Revenue by Type</div>
            <ResponsiveContainer width="100%" height={140}>
              <PieChart>
                <Pie data={typeData} cx="50%" cy="50%" outerRadius={55} dataKey="value" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false} fontSize={10}>
                  {typeData.map((_,i)=><Cell key={i} fill={PIE_COLORS[i%PIE_COLORS.length]}/>)}
                </Pie>
                <Tooltip contentStyle={{background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:9,fontSize:12}} formatter={v=>[`₹${v.toLocaleString("en-IN")}`,""]}/>
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
...
function AppShell({ user, ytUser, ytData, ytLoading, ytError, signIn, signOut }) {
  ...
  const PAGES = {
    ...
    collab: <CollabPage/>,
    invoice: <InvoicePage profile={profile}/>,
    earn: <EarnPage/>,
  };
    ...
    <div className="app">
      <div className="sidebar">
        ...
        <div className="plat-row">
          {ytUser&&<span className="pp yt">▶️ {ytUser.handle||"YouTube"}</span>}
          {!ytUser&&<span style={{fontSize:11,color:"var(--t3)"}}>No platform connected</span>}
        </div>
      </div>
      <div className="main">{PAGES[page]||PAGES.dash}</div>
    </div>
  );
}
